...
public class BaseScene extends Scene {
    ...
    protected BaseScene() {
        super();
        Shape p1 = new Plane(ColorRGB.blue, ColorRGB.lightGray, 20D, new Vec3f(0D, 0D, 1D), 6D);
        Shape p2 = new Plane(ColorRGB.green, ColorRGB.lightGray, 20D, new Vec3f(1D, 0D, 0D), 3D);
        Shape p3 = new Plane(ColorRGB.red, ColorRGB.lightGray, 20D, new Vec3f(-1D, 0D, 0D), 3D);

        shapes.add(p1);
        shapes.add(p2);
        shapes.add(p3);

        Ray3D light = new Ray3D(new Vec3f(1D, 1D, 0D), ColorRGB.white, ColorRGB.lightGray);
